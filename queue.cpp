...
void* add_B(void*)
{
    ...
    while (1) {
        ...
        else if (consumerCount3 +  consumerCount2 + consumerCount1 == MAX) {
            pthread_mutex_unlock(&mutex);
            return NULL;
        }
...
int main()
{
    ...
    if (sum_C > sum_B && sum_C > sum_D )
        cout << "Winner is Thread C" << endl;
    else if (sum_B > sum_C && sum_B >  sum_D)
        cout << "Winner is Thread B" << endl;
    else if (sum_D > sum_B && sum_D > sum_C)
        cout << "Winner is Thread D" << endl;
    else
